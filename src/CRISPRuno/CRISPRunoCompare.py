...
def compareCRISPRuno(arg_els):

    logger = logging.getLogger('CRISPRunoCompare')
    ...
    parser = argparse.ArgumentParser(description='CRISPRunoCompare: Compares two runs from CRISPRuno', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--folder1_root', '-f1', type=str, help='Root of the first completed CRISPRuno analysis', required=True)
    parser.add_argument('--folder1_name', '-n1', type=str, help='Name of the first completed CRISPRuno analysis')
    parser.add_argument('--folder2_root', '-f2', type=str, help='Root of the second completed CRISPRuno analysis', required=True)
    parser.add_argument('--folder2_name', '-nl', type=str, help='Name of the second completed CRISPRuno analysis')
    parser.add_argument('--output_root', '-o', type=str, help='Root of the second completed CRISPRuno analysis', required=True)
...
def parse_frag_tx_list(filename):
    """
    Parses a fragment translocation list file from a completed CRISPRuno run
    ...
    Returns:
        ...
        cut_annotations: dict of cut>annotation (e.g. chr1:234:left > 'Novel') Note that in CRISPRuno the key for this dict is only 'chr1:234' instead of 'chr1:234:left'
...
def plot_tx_list(f1_final_cut_counts, f1_cut_annotations, f1_cut_classification_lookup, f2_final_cut_counts,
    ...
    requires two dicts for each finished CRISPRuno run:
        final_cut_counts: dict of cut>count (e.g. chr1:234:left > 5)
        cut_annotations: dict of cut>annotation (e.g. chr1:234:left > 'Novel') Note that in CRISPRuno the key for this dict is only 'chr1:234' instead of 'chr1:234:left'
