...
import CRISPRuno.CRISPRunoCore
import multiprocessing
import multiprocessing.pool
import logging

def processBatch(arg_els):
    usage_str = 'Usage: CRISPRunoBatch.py {batch file} {settings file} {?--batch_processes int} {additional settings}'
    ...
    logger = logging.getLogger('CRISPRunoBatch')
    ...
    #logger for sub runs
    sub_logger = logging.getLogger('CRISPRuno')
    fh = logging.FileHandler(batch_file+'.CRISPRunoBatch.log')
    ...
    logger.info('Printing sub-command logs to: ' + batch_file+'.CRISPRunoBatch.log')
    ...
    with open(batch_file,'r') as fin:
        ...
        for line in fin:
            ...
            try:
                ...
                settings = CRISPRuno.parse_settings(this_command_args)
                settings_arr.append(settings)
            except Exception as e:
                raise Exception('Error in parsing ' + batch_file + ' line ' + str(line_idx)+ ":\n" + str(e)) from e

    logger.info('Running CRISPRuno with %d processes'%batch_process_count)
    result_summary_files = []
    with NonDaemonPool(processes=batch_process_count) as pool:
        result_summary_files = pool.map(runOneCRISPRuno,settings_arr)
...
def runOneCRISPRuno(settings):
    logger = logging.getLogger('CRISPRunoBatch')
    logger.info('Running ' + settings['root'])
    final_file = CRISPRuno.processCRISPRuno(settings)
